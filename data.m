    ...
    methods
        ...
        function obj = add_value(obj,cyclecounter, blob)
            switch obj.datatype %% ENUMERATING to prevent String compares during live loop
                case 0 %int_8
                    obj.values(cyclecounter) = blob;
                case 1 %uint_8
                    obj.values(cyclecounter) = typecast(blob,'uint8');
                case 2 %int_16
                    obj.values(cyclecounter) = typecast(blob(1:2),'int16');
                case 3 %uint_16
                    obj.values(cyclecounter) = typecast(blob(1:2),'uint16');
                case 4 %int_32
                    obj.values(cyclecounter) = typecast(blob(1:4),'int32');
                case 5 %uint_32
                    obj.values(cyclecounter) = typecast(blob(1:4),'uint32');
                case 6 %float_32
                    obj.values(cyclecounter) = typecast(blob(1:4),'single');
                case 7 %int_64
                    obj.values(cyclecounter) = typecast(blob(1:8),'int64');
                case 8 %uint_64
                    obj.values(cyclecounter) = typecast(blob(1:8),'uint64');
                case 9 %float_64
                    obj.values(cyclecounter) = typecast(blob(1:8),'double');
            end
