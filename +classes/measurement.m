    ...
    methods
        ...
        %password.
           ...
           sqlquery = ['SELECT `STP_measurements`.`id`, ' ...
                    ...
                    '       `STP_measurements`.`count`, ' ...
                    '       `STP_measurements`.`max` ' ...                    
        ...
        function obj = declaration(obj,date,dur)
           ...
           sqlquery = ['SELECT MAX(`STP_measurement_dataset`.`cyclecounter`) AS `max` ' ...
            ...
            % Selecting setup
            sqlquery = ['SELECT `STP_instrument_parameter_values`.`value`, '...
            '       `STP_instruments`.`id`, '...
            '       `STP_instruments`.`name`, '...
            '       `STP_instruments`.`description` '...
            'FROM `STP_instrument_parameter_values` '...
            'INNER JOIN `STP_instruments` '...
            ' ON `STP_instruments`.`id` = `STP_instrument_parameter_values`.`instrument_id` '...
            ...
            'WHERE `STP_instrument_parameter_values`.`parameter_id` = 33 '...
            ' AND `STP_setups`.`id` = 110 '...
...
%                 'ORDER BY `STP_instruments`.`id` ASC;'];
            ...
            'FROM ( '...
            'SELECT `STP_instrument_type_parameter_values`.`value`, '...
            '       `STP_instruments`.`id`, '...
            '       `STP_instruments`.`name`, '...
            '       `STP_instruments`.`description` '...
            'FROM `STP_instrument_type_parameter_values` '...
            ...
            ') AS `t1` '...
            'LEFT JOIN ( '...
            'SELECT `STP_instrument_parameter_values`.`value`, '...
            '       `STP_instruments`.`id` '...
            'FROM `STP_instrument_parameter_values` '...
            'INNER JOIN `STP_instruments` '...
            ' ON `STP_instruments`.`id` = `STP_instrument_parameter_values`.`instrument_id` '...
